...
_SKIP_FIELDS = frozenset({'id', 'tipo', 'status', 'versao', 'autor', 'data-criacao',
                           'data-atualizacao', 'tags', 'titulo', 'descricao', 'notas'})
...
def parse_file(path: Path) -> Entity:
    ...
    stem_prefix = entity_id.split('-')[0] if '-' in entity_id else path.stem
    tipo = str(meta.get('tipo', stem_prefix))
