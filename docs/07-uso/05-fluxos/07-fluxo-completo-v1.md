...
## Resultado esperado de toda a sessão

```bash
# Estado do banco ao final
sqlite3 banco_de_dados/sqlite/soe-ccg.db "
  SELECT 'receitas' as tipo, COUNT(*) FROM receitas UNION ALL
  ...
  SELECT 'execucoes', COUNT(*) FROM execucoes UNION ALL
  SELECT 'observacoes', COUNT(*) FROM observacoes;
"
```

```
# Resultado esperado (partindo do estado seed inicial):
receitas|2
ingredientes|9
tecnicas|4
equipamentos|3
...
```
# Resultado esperado:
a1b2c3d feat: sessao 2026-06-27 — REC-000002 pao-de-queijo + 5 ING + EXE + OBS
```

---

## Problemas comuns na sessão completa

| Problema | Causa | Solução |
|----------|-------|---------|
| FAA reprovado no final mas não no início | Um novo arquivo tem problema | `auditor-v1.py issues --critical` para identificar |
| `referencia_quebrada` ao importar a REC | ING ou TEC criado mas não importado ainda | Importar cada entidade imediatamente após criar |
| `git status` mostra arquivos inesperados | `soe-ccg.db` não está no `.gitignore` | Verificar `.gitignore` — o banco nunca deve ser commitado |

---

## Próxima leitura

- Sessão mais curta (criar só um tipo de entidade) → [`../10-cookbook/`](../10-cookbook/)
- Algo deu errado → [`../09-troubleshooting/README-v1.md`](../09-troubleshooting/README-v1.md)
