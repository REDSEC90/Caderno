...
sqlite3 banco_de_dados/sqlite/soe-ccg.db \
  "SELECT id, status FROM execucoes WHERE receita_id = 'REC-000001' ORDER BY id;"
```

```
# Resultado esperado:
EXE-000001|concluida
EXE-000002|concluida
```
...
|----------|-------|---------|
| `referencia_quebrada: REC-000001` | A receita referenciada não existe | Verificar o `receita-id` no frontmatter da EXE |
| EXE importada mas não aparece na consulta da receita | O campo `receita-id` está ausente ou com nome errado | O campo correto é `receita-id`, não `receita` ou `recipe-id` |
| Avaliação não persiste no banco | Campo de avaliação pode não estar mapeado no schema | Verificar `banco_de_dados/esquemas/schema-sqlite-v1.sql` |

---

## Próxima leitura

- Criar uma observação a partir do que foi aprendido → [`05-criar-observacao-v1.md`](05-criar-observacao-v1.md)
- Criar um experimento para testar uma hipótese → [`04-criar-experimento-v1.md`](04-criar-experimento-v1.md)
- Versão rápida → [`../10-cookbook/05-criar-execucao-v1.md`](../10-cookbook/05-criar-execucao-v1.md)
