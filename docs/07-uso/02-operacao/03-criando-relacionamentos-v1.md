...
## Por que os tipos de aresta existem

A distinção entre STRUCTURAL, COMPOSITIONAL e INFORMATIONAL não é burocrática — ela determina o que o Validador trata como erro. Um ciclo entre arestas STRUCTURAL é sempre um problema de modelagem (dependência impossível de resolver). Um ciclo entre arestas INFORMATIONAL é normal e esperado (menções cruzadas entre documentos).

Colocar um ID no frontmatter (gera STRUCTURAL ou COMPOSITIONAL) versus no corpo do texto (gera INFORMATIONAL) é uma decisão semântica com consequências reais na validação.

---

## Próxima leitura

- Como o Resolver verifica estas arestas → [`../03-validacao/03-resolver-v1.md`](../03-validacao/03-resolver-v1.md)
- Navegação no grafo via consultas → [`../04-consultas/03-navegacao-no-grafo-v1.md`](../04-consultas/03-navegacao-no-grafo-v1.md)
