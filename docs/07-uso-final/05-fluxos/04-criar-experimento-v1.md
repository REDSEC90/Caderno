...
sqlite3 banco_de_dados/sqlite/soe-ccg.db \
  "SELECT id, status FROM experimentos WHERE id = 'EXP-000001';"
```

```
# Resultado esperado (experimento aberto):
EXP-000001|aberto

# Resultado esperado (após conclusão):
EXP-000001|incorporado
```

```bash
# Verificar aresta DERIVATION para a receita base
sqlite3 banco_de_dados/sqlite/soe-ccg.db \
  "SELECT target, kind FROM relacionamentos WHERE source = 'EXP-000001';"
```

```
# Resultado esperado:
REC-000001|DERIVATION
```
...
|----------|-------|---------|
| `referencia_quebrada: REC-000001` | Campo `receita-base-id` incorreto | Verificar que o ID da receita base existe |
| Aresta DERIVATION não aparece | Campo `receita-base-id` com nome incorreto | O campo canônico é exatamente `receita-base-id` |
| EXP isolado no FAA | Sem `receita-base-id` no frontmatter | Adicionar o campo e reimportar |

---

## Próxima leitura

- Criar a nova versão da receita com o resultado incorporado → [`06-atualizar-receita-v1.md`](06-atualizar-receita-v1.md)
