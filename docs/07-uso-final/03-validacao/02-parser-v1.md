...
## Por que o Parser existe como etapa separada

O Parser é separado do Resolver por uma razão: um arquivo pode ser bem-formado (Parser passa) mas ter referências quebradas (Resolver falha). Separar as etapas permite distinguir os dois tipos de erro — e permite importar um lote de arquivos com relatório completo de todos os que têm referências quebradas, em vez de parar no primeiro.
...
```bash
python3 codigo/parser-v1.py dados/ingredientes/ING-000001-leite-integral-v1.md
```

```
# Resultado esperado:
[PARSER] ING-000001 processado — 0 arestas de saída
```
...
```
# Resultado esperado:
[PARSER] REC-000001 processado — 8 arestas (4 ING, 2 TEC, 2 EQP via COMPOSITIONAL)
```
...
- O que o Resolver faz depois → [`03-resolver-v1.md`](03-resolver-v1.md)
- Troubleshooting do Parser → [`../09-troubleshooting/01-parser-v1.md`](../09-troubleshooting/01-parser-v1.md)
