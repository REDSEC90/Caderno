...
## Por que o Resolver existe como etapa separada

O Parser extrai tudo que está declarado — é otimista. O Resolver verifica a realidade — é cético. Essa separação permite ao sistema distinguir claramente: "este arquivo está malformado" (erro de Parser) de "este arquivo está correto mas referencia algo inexistente" (erro de Resolver). São erros de natureza diferente que exigem correções diferentes.
...
```bash
python3 codigo/resolvedor-v1.py dados/
```

```
# Resultado esperado (sistema consistente):
[RESOLVER] 13 entidades, 21 arestas — 0 referências quebradas
```

```
# Resultado com problema:
[RESOLVER] referencia_quebrada: REC-000002 → ING-000099 (não existe)
  1 erro encontrado
```
...
- O que o Validador faz depois → [`04-validador-v1.md`](04-validador-v1.md)
- Troubleshooting do Resolver → [`../09-troubleshooting/02-resolver-v1.md`](../09-troubleshooting/02-resolver-v1.md)
