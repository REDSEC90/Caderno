# Contrato Formal: Ingrediente

**Entidade:** Ingrediente  
**Versão do contrato:** v1  
**Esquema associado:** `docs/01-dominio/esquemas/esquema-ingrediente-v1.md`  
**Especificação associada:** `docs/01-dominio/especificacao-ingrediente.md`
...
|-------|------|-----------|----------------------------|
| `id` | string | `ING-NNNNNN` | "ID ausente ou em formato inválido" |
| `tipo` | string | valor fixo: `ingrediente` | "Tipo de entidade inválido" |
...
| `nome` | string | não vazio | "Nome do ingrediente ausente" |
...
|-------|------|-----------|--------------------------|
| `tipo-ingrediente` | string | valores do catálogo oficial | Tratado como null |
| `unidade-padrao` | string | valores do catálogo de unidades | Tratado como null |
| `descricao` | string | — | Tratado como null |
| `sazonalidade` | string | — | Tratado como null |
...
- [ ] `tipo-ingrediente` existe no catálogo (se presente)
- [ ] `unidade-padrao` existe no catálogo (se presente)
...
- [ ] Registro existe no sistema
- [ ] `status` = `arquivado`
- [ ] Nenhum Ingrediente ativo com mesmo `id`
...
**Criar:**
- [ ] Registro existe em `dados/ingredientes/[id].md`
...
- [ ] Receitas e Execuções históricas que referenciam o Ingrediente permanecem válidas
...
7. Ingrediente pode existir sem ser referenciado por nenhuma Receita
8. `atualizado-em` ≥ `criado-em` sempre
9. Arquivo Markdown em `dados/ingredientes/` é a única fonte da verdade
...
| Criar com `id` duplicado | Violação da unicidade | "ID ING-NNNNNN já existe no sistema" |
| Atribuir `tipo-ingrediente` inválido | Violação do catálogo | "Tipo de ingrediente não encontrado no catálogo" |
| Atribuir `unidade-padrao` inválida | Violação do catálogo | "Unidade não encontrada no catálogo" |
...
1. Cria, atualiza e arquiva Ingredientes respeitando todas as pré e pós-condições
2. Mantém todas as invariantes após qualquer operação
3. Rejeita todas as operações listadas em "Rejeições Explícitas"
4. Aceita ingredientes válidos de todas as versões de esquema documentadas
5. Não perde informação entre Markdown e SQLite
6. Preserva histórico de estados como append-only
7. Garante que Markdown em `dados/ingredientes/` é sempre a fonte da verdade
8. Permite que Ingrediente exista sem nenhuma Receita que o referencie
