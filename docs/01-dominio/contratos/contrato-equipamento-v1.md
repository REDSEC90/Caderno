# Contrato Formal: Equipamento

**Entidade:** Equipamento  
**Versão do contrato:** v1  
**Esquema associado:** `docs/01-dominio/esquemas/esquema-equipamento-v1.md`  
**Especificação associada:** `docs/01-dominio/especificacao-equipamento.md`
...
|-------|------|-----------|----------------------------|
| `id` | string | `EQP-NNNNNN` | "ID ausente ou em formato inválido" |
| `tipo` | string | valor fixo: `equipamento` | "Tipo de entidade inválido" |
...
| `nome` | string | não vazio | "Nome do equipamento ausente" |
...
|-------|------|-----------|--------------------------|
| `tipo-equipamento` | string | valores do catálogo oficial | Tratado como null |
| `descricao` | text | — | Tratado como null |
| `capacidade` | string | — | Tratado como null |
| `material` | string | — | Tratado como null |
...
- [ ] `tipo-equipamento` existe no catálogo (se presente)
...
- [ ] Registro existe no sistema
- [ ] `status` = `arquivado`
- [ ] Nenhum Equipamento ativo com mesmo `id`
...
**Criar:**
- [ ] Registro existe em `dados/equipamentos/[id].md`
...
- [ ] Receitas e Execuções históricas que referenciam o Equipamento permanecem válidas
...
7. Equipamento pode existir sem ser referenciado por nenhuma Receita
8. `atualizado-em` ≥ `criado-em` sempre
9. Arquivo Markdown em `dados/equipamentos/` é a única fonte da verdade
...
| Criar com `id` duplicado | Violação da unicidade | "ID EQP-NNNNNN já existe no sistema" |
| Atribuir `tipo-equipamento` inválido | Violação do catálogo | "Tipo de equipamento não encontrado no catálogo" |
...
1. Cria, atualiza e arquiva Equipamentos respeitando todas as pré e pós-condições
2. Mantém todas as invariantes após qualquer operação
3. Rejeita todas as operações listadas em "Rejeições Explícitas"
4. Aceita equipamentos válidos de todas as versões de esquema documentadas
5. Não perde informação entre Markdown e SQLite
6. Preserva histórico de estados como append-only
7. Garante que Markdown em `dados/equipamentos/` é sempre a fonte da verdade
8. Permite que Equipamento exista sem nenhuma Receita que o referencie
